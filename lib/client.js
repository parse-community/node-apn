...
module.exports = function (dependencies) {
  ...
  Client.prototype.write = function write(method, path, notification, count) {
    if (this.isDestroyed) {
    ...
    if (!this.session || this.session.closed || this.session.destroyed) {
      return this.connect().then(() => this.request(method, path, notification, count));
    }
    return this.request(method, path, notification, count);
  };
  ...
  Client.prototype.request = function reqest(method, path, notification, count) {
        ...
        try {
          ...
          if (status === 200) {
            ...
            if (status === 403 && response.reason === 'ExpiredProviderToken' && retryCount < 2) {
              ...
              resolve(this.write(method, path, notification, retryCount + 1));
              return;
