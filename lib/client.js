...
module.exports = function (dependencies) {
  ...
  Client.prototype.writeV2 = function writeV2(method, path, notification, count) {
    if (this.isDestroyed) {
      return Promise.resolve({ method, path, error: new VError('client is destroyed') });
    }

    // Connect session
    if (!this.session || this.session.closed || this.session.destroyed) {
      return this.connect().then(() => this.requestV2(method, path, notification, count));
    }
    return this.requestV2(method, path, notification, count);
  };
  ...
  Client.prototype.requestV2 = function reqestV2(method, path, notification, count) {
    let tokenGeneration = null;
    let status = null;
    let responseData = '';
    const retryCount = count || 0;
    ...
    return new Promise(resolve => {
      request.on('end', () => {
        try {
          ...
          if (status === 200) {
            ...
            if (status === 403 && response.reason === 'ExpiredProviderToken' && retryCount < 2) {
              this.config.token.regenerate(tokenGeneration);
              resolve(this.writeV2(method, path, notification, retryCount + 1));
              return;
            } else if (status === 500 && response.reason === 'InternalServerError') {
              this.closeAndDestroySession();
              const error = new VError('Error 500, stream ended unexpectedly');
              resolve({ method, path, error });
              return;
            }

            resolve({ method, path, status, response });
          } else {
            this.closeAndDestroySession();
            const error = new VError(
              `stream ended unexpectedly with status ${status} and empty body`
      ...
      request.setTimeout(this.config.requestTimeout, () => {
        ...
        resolve({ method, path, error: new VError('apn write timeout') });
      });
      ...
      request.on('aborted', () => {
        ...
        resolve({ method, path, error: new VError('apn write aborted') });
      });
